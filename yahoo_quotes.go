...
type Stock struct {
	...
	AfterHoursPrice     string `json:"postMarketPrice"`         // k2: percent change real time.
	AfterHoursChange    string `json:"postMarketChange"`        // k2: percent change real time.
	AfterHoursChangePct string `json:"postMarketChangePercent"` // k2: percent change real time.
...
func (quotes *Quotes) parse2(body []byte) (*Quotes, error) {
	...
	for i, raw := range results {
		...
		quotes.stocks[i].AfterHoursPrice = result["postMarketPrice"]
		quotes.stocks[i].AfterHoursChange = result["postMarketChange"]
		quotes.stocks[i].AfterHoursChangePct = result["postMarketChangePercent"]
